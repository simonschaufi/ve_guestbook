...
class tx_veguestbook_pi1 extends AbstractPlugin
{
    ...
    public function init($conf)
    {
        ...
        $this->sys_language_uid = $this->frontendController->config['config']['sys_language_uid'] ? $this->frontendController->config['config']['sys_language_uid'] : '0';
        ...
        $pid_list = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'pages', 'sDEF') ? implode(GeneralUtility::intExplode(',', $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'pages', 'sDEF')), ',') : $this->frontendController->id;
        ...
        $this->config['redirect_page'] = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'redirect_page', 's_form') ? implode(GeneralUtility::intExplode(',', $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'redirect_page', 's_form')), ',') : $this->frontendController->id;
        ...
        $this->config['email_subst'] = $this->frontendController->tmpl->setup['config.']['spamProtectEmailAddresses_atSubst'];
        ...
        if (ExtensionManagementUtility::isLoaded('sr_freecap') && $this->config['captcha'] == 'sr_freecap') {
    ...
    public function displayList()
    {
        ...
        if ($this->code == 'TEASER') {
            ...
            if ($this->config['guestbook'] > 0) {
                ...
                $wrappedSubpartArray = [];
                $wrappedSubpartArray['###LINK_ITEM###'] = explode('|', $this->pi_linkTP_keepPIvars('|', [], 1, '', $this->config['guestbook']));
            } else {
        ...
        if ($count > 0) {
            ...
            $entry = $this->frontendController->cObj->getSubpart($list, '###ENTRY###');
            ...
            if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ve_guestbook']['extraItemMarkerHook'])) {
                foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ve_guestbook']['extraItemMarkerHook'] as $_classRef) {
                    ...
                    $markerArray = $_procObj->extraItemMarkerProcessor($markerArray, [], $this->config, $this);
                }
            }
            ...
            return $this->cObj->substituteMarkerArrayCached($no_entries, $markerArray, [], []);
        }
    }
     ...
     */
    protected function getListContent($res, $templatecode)
    {
        $entries = '';
        $cc = null;
        while ($row = $this->databaseConnection->sql_fetch_assoc($res)) {
            ...
            // hook for processing of extra subparts
            ...
            $entries .= $this->cObj->substituteMarkerArrayCached($current_templatecode, $markerArray, [], []);
     ...
     */
    protected function getItemMarkerArray($row)
    {
     ...
     */
    protected function cutDown($text)
    {
     ...
     */
    protected function get_url($tag = '', $id, $vars = [], $unsetVars = [], $usePiVars = true)
    {
        $vars = (array)$vars;
     ...
     * @return array
     */
    protected function initFormMarkerArray()
    {
        $markerArray = [];
        ...
        if (is_array($this->LOCAL_LANG) && count($this->LOCAL_LANG) > 0) {
            ...
            if (isset ($this->postvars['submitted']) && $this->postvars['submitted'] == 1) {
                ...
                if (!empty ($error)) {
                    ...
                    $db_fields = ['firstname', 'surname', 'email', 'homepage', 'place', 'entry', 'entrycomment'];

                    $saveData['pid'] = $this->config['pid_list'];
                    $saveData['tstamp'] = $GLOBALS['EXEC_TIME'];
                    $saveData['crdate'] = $GLOBALS['EXEC_TIME'];
                    $saveData['deleted'] = 0;
                    ...
                    if ($insert) {
                        ...
                        if ($this->config['clearCacheCmdOnInsert']) {
                            $commands = GeneralUtility::trimExplode(',', strtolower($this->config['clearCacheCmdOnInsert']), 1);
                            $commands = array_unique($commands);
                            foreach ($commands as $commandPart) {
                                $this->frontendController->clearPageCacheContent_pidList($commandPart);
                            }
                        }

                        $this->frontendController->clearPageCacheContent_pidList($this->frontendController->id);
                        $this->frontendController->clearPageCacheContent_pidList($this->config['redirect_page']);

                        // clear index
                        if (ExtensionManagementUtility::isLoaded('indexed_search')) {
                            $res = $this->databaseConnection->exec_SELECTquery('phash', 'index_phash', 'data_page_id = ' . $this->config['redirect_page']);
                            ...
                            if ($res) {
                                ...
                                while ($row = $this->databaseConnection->sql_fetch_assoc($res)) {
                                    $phash = (int)$row['phash'];
                                    if ($phash > 0) {
                                        $indexer->removeOldIndexedPages($phash);
                                    }
                                }
                            }
                        }
                        HttpUtility::redirect(GeneralUtility::getIndpEnv('TYPO3_REQUEST_DIR') . $this->getUrl($this->config['redirect_page']));
                    }
                }
            }
            ...
            if (!$this->postvars && $this->frontendController->loginUser) {
                ...
                $markerArray['###VALUE_EMAIL###'] = $this->frontendController->fe_user->user['email'];
                $markerArray['###VALUE_HOMEPAGE###'] = $this->frontendController->fe_user->user['www'];
                $markerArray['###VALUE_PLACE###'] = $this->frontendController->fe_user->user['city'];
            }
            ...
            if (!$this->frontendController->loginUser) {
                ...
                } elseif (ExtensionManagementUtility::isLoaded('captcha') and $this->config['captcha'] == 'captcha') {
            ...
            $form = $this->cObj->substituteMarkerArrayCached($template, $markerArray, [], []);
     ...
     */
    protected function getAddParams($addParamsList)
    {
        $addParamArray = [];
     ...
     */
    protected function sendNotificationMail($emailto)
    {
        ...
        $markerArray['###URL###'] = $this->cObj->typoLink(
            GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $this->getUrl($this->config['guestbook']),
            [
                'parameter' => $this->config['guestbook'],
        ...
        if (is_array($this->postvars)) {
            ...
            $notification_mail_subject = $this->cObj->substituteMarkerArrayCached($notification_mail_subject, $markerArray, [], []);
            $notification_mail_text = $this->cObj->substituteMarkerArrayCached($notification_mail_text, $markerArray, [], []);
            ...
            $mailer->setTo($emailto)
                ...
                ->setFrom($emailfrom, $this->getEmailFromName())
                ->setBody($notification_mail_text, 'text/html')
     ...
     * @return    boolean        Mail delivery: true / false
     */
    protected function sendFeedbackMail($emailto)
    {
     ...
     */
    protected function markObligationFields($markerArray)
    {
        if (is_array($this->config['obligationfields']) && count($this->config['obligationfields']) > 0) {
     ...
     */
    protected function substituteEmoticons($str)
    {
